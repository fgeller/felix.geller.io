goog.provide("module$resources$public$js$libs$codemirror_5_5$lib$codemirror");
var exports$$module$resources$public$js$libs$codemirror_5_5$lib$codemirror = module$resources$public$js$libs$codemirror_5_5$lib$codemirror;
(function(mod) {
  module$resources$public$js$libs$codemirror_5_5$lib$codemirror = mod();
})(function() {
  ...
  function CodeMirror(place, options) {
    ...
    initScrollbars(this);
    this.state = {keyMaps:[], overlays:[], modeGen:0, overwrite:false, delayingBlurEvent:false, focused:false, suppressEdits:false, pasteIncoming:false, cutIncoming:false, draggingText:false, highlight:new Delayed, keySeq:null, specialChars:null};
    var cm = this;
    if (ie && ie_version < 11) {
      setTimeout(function() {
  ...
  NullScrollbars.prototype = copyObj({update:function() {
  ...
  function applyTextInput(cm, inserted, deleted, sel, origin) {
    ...
    for (var i = sel.ranges.length - 1;i >= 0;i--) {
      ...
      var changeEvent = {from:from, to:to, text:multiPaste ? multiPaste[i % multiPaste.length] : textLines, origin:origin || (paste ? "paste" : cm.state.cutIncoming ? "cut" : "+input")};
  ...
  TextareaInput.prototype = copyObj({init:function(display) {
    ...
    te.style.cssText = "position: fixed; width: 30px; height: 30px; top: " + (e.clientY - 5) + "px; left: " + (e.clientX - 5) + "px; z-index: 1000; background: " + (ie ? "rgba(255, 255, 255, .05)" : "transparent") + "; outline: none; border-width: 0; outline: none; overflow: hidden; opacity: .05; filter: alpha(opacity\x3d5);";
    if (webkit) {
  ...
  }, setUneditable:nothing, needsContentAttribute:false}, TextareaInput.prototype);
  ...
  ContentEditableInput.prototype = copyObj({init:function(display) {
  ...
  }, onContextMenu:nothing, resetPosition:nothing, needsContentAttribute:true}, ContentEditableInput.prototype);
  ...
  function startOperation(cm) {
    cm.curOp = {cm:cm, viewChanged:false, startHeight:cm.doc.height, forceUpdate:false, updateInput:null, typing:false, changeObjs:null, cursorActivityHandlers:null, cursorActivityCalled:0, selectionChanged:false, updateMaxLine:false, scrollLeft:null, scrollTop:null, scrollToPos:null, focus:false, id:++nextOpId};
    if (operationGroup) {
      operationGroup.ops.push(cm.curOp);
    } else {
      cm.curOp.ownsGroup = operationGroup = {ops:[cm.curOp], delayedCallbacks:[]};
    }
  }
  ...
  function registerEventHandlers(cm) {
    ...
    d.dragFunctions = {simple:function(e) {
      if (!signalDOMEvent(cm, e)) {
        e_stop(e);
      }
    }, start:function(e) {
  ...
  function leftButtonSelect(cm, e, start, type, addNew) {
    ...
    if (!addNew) {
      ...
      if (ourIndex == -1) {
        ...
        if (ranges.length > 1 && ranges[ourIndex].empty() && type == "single" && !e.shiftKey) {
  ...
  function onDragStart(cm, e) {
    ...
    if (e.dataTransfer.setDragImage && !safari) {
      ...
      img.src = "data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw\x3d\x3d";
      if (presto) {
  ...
  function filterChange(doc, change, update) {
    var obj = {canceled:false, from:change.from, to:change.to, text:change.text, origin:change.origin, cancel:function() {
      this.canceled = true;
    }};
    if (update) {
  ...
  "Shift-Ctrl-R":"replaceAll", "Ctrl-[":"indentLess", "Ctrl-]":"indentMore", "Ctrl-U":"undoSelection", "Shift-Ctrl-U":"redoSelection", "Alt-U":"redoSelection", fallthrough:"basic"};
    ...
    while (!stream.eol()) {
      ...
      if (!flattenSpans || curStyle != style) {
        while (curStart < stream.start) {
          curStart = Math.min(stream.start, curStart + 5E4);
          f(curStart, curStyle);
        }
        curStyle = style;
      }
  ...
  function interpretTokenStyle(style, options) {
    ...
    return cache[style] || (cache[style] = style.replace(/\S+/g, "cm-$\x26"));
  }
  function buildLineContent(cm, lineView) {
    var content = elt("span", null, null, webkit ? "padding-right: .1px" : null);
    var builder = {pre:elt("pre", [content], "CodeMirror-line"), content:content, col:0, pos:0, cm:cm, splitSpaces:(ie || webkit) && cm.getOption("lineWrapping")};
    lineView.measure = {};
      ...
      if (nextChange == pos) {
        ...
        for (var j = 0;j < spans.length;++j) {
          ...
          if (m.type == "bookmark" && sp.from == pos && m.widgetNode) {
            ...
            if (sp.from <= pos && (sp.to == null || sp.to > pos || m.collapsed && sp.to == pos && sp.from == pos)) {
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {constructor:Doc, iter:function(from, to, op) {
    ...
    var realOpts = {replacedWith:options && (options.nodeType == null ? options.widget : options), insertLeft:options && options.insertLeft, clearWhenEmpty:false, shared:options && options.shared, handleMouseEvents:options && options.handleMouseEvents};
